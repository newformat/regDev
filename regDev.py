...
# 27.11.19 - идея, после переофрмления кода в ООП перевести списки в словарь, так будет лучше, пример:
...
class DataWriteForLists:
    ...
    def add_leave_time_set(self):
        for listMAC in self.list_regDevice:
            self.list_leaveTimeSet.append( "CALL ZocSend \"zb.sysopt.leaveTimeSet " + listMAC[54:70] + " " + str(self.leaveTime) + "\"" + str(self.template_new))
...
class DataWriteForFile:
    ...
    def file_write(self):
        try:
            # в перспективах - разделить запись данных - шаблон(начало файла) : список адресов : шаблон(конец файла)
...
# запись анализа данных в файл статистики .thm
class DataAnalysis:
